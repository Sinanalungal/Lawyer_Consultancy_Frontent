...
const ChatComponent = ({ send_to_id, path }) => {
    ...
    if (thread) {
      const fetchThread = async () => {
        try {
          ...
          const response = await axiosInstance.get(`${BASE_URL}chat/messages/thread/${thread.id}/`);
  ...
  useEffect(() => {
    ...
    newSocket.onmessage = (e) => {
      ...
      console.log(receivedMessage,'latest recieved msg');
  ...
  const handleSend = () => {
    if (input.trim() && socket) {
      let data = { sent_by: value, send_to: Number(send_to_id), text: input, thread: thread.id };
  ...
  return (
    <div className='max-w-[1400px] mx-auto'>
      <div className='flex'>
        ...
        {thread ? (
          <div className='flex ml-1 flex-col max-sm:hidden h-96 mx-auto w-[90%] border border-gray-300 rounded-lg'>
              ...
              {messages.map((msg, index) => {
                ...
                return (
                  msg.thread === thread.id && (
                    <div
                      key={index}
                      className={`flex flex-col mb-2 ${msg.send_by === value ? 'items-end' : 'items-start'}`}
                    >
                      <div
                      ...
                      <p className='text-[7px] pt-[2px] pl-1'>{formattedTime}</p>
                    </div>
                  )
                );
              })}
